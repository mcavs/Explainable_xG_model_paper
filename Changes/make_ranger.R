...
make_ranger <- function(data, target, type, fill_na = FALSE, num_features = NULL, tune = FALSE, tune_metric = NULL, tune_iter = 20, label = "Ranger", refclass = NULL) {
  ...
  form <- stats::as.formula(paste(target , "~.")) #changed

  is_classif <- type == "classification"

  if (!tune){
    rg <- ranger::ranger(form, data = data, classification = is_classif, probability = TRUE) #changed
  } else {
  ...
  ranger_predict <- function(object, newdata) {
    ...
    if(is_classif == TRUE){#changed
      if(is.null(refclass)){
        preds <- predict(object, newdata)$predictions[,2]
        } else {preds <- predict(object, newdata)$predictions[,1]}#changed
    } else {preds <- predict(object, newdata)$predictions}
    return(preds)
  }
